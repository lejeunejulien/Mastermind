...
class Insert_victoire_defaiteBSDsimple:
    def __init__(self,victoire=0,defaite=0):
        ...
        liste=cur.fetchall()[len(cur.fetchall())-1]
...
class Consulter_infos(BSDpartie_simple):
    def __init__(self):
        ...
        self.label4.pack(anchor=N,padx=45,pady=5)
        
        self.label5=Label(fen2,text='Victoire(s) : ')
        self.label5.pack(anchor=N,padx=45,pady=5)
        
        self.label6=Label(fen2,text='Défaite(s) : ')
        self.label6.pack(anchor=N,padx=45,pady=5)

        self.labelsup1=Label(fen2)
        self.labelsup1.pack(anchor=N,padx=45,pady=5)

        self.labelsup2=Label(fen2)
        self.labelsup2.pack(anchor=N,padx=45,pady=5)
        
        fen2.pack(side=LEFT,anchor=N,pady=80,padx=80)

        Button(self.fen,text='Ok',padx=40,pady=6,fg='orange',bg='black',\
               command=self.fen.destroy).pack(side=BOTTOM,anchor=W,pady=20,padx=4)
    ...
    def selection_liste(self,event):
        selection_liste=self.liste.get(self.liste.curselection())
        if self.selection=="Parties simples":
            if self.cherche_parties(selection_liste)!=[]:
                for i in self.cherche_parties(selection_liste):
        ...
        if self.selection=="Carrières":
            if existence_base(0):
                ...
                if self.cur.fetchall()!=[]:
                    elems=selection_liste.split()
                    self.cur.execute("select nom, prenom, nbrevictoires, nbredefaites, nationalite,\
age from identite where nom='%s' and prenom='%s'"%(elems[0],elems[1]))
                    for i in self.cur.fetchall():
...
class Joueur2:
    def __init__(self):
        ...
        font6=Font(family='Courier',size=13,weight='bold')
...
class Jeu(Deplacement):
    def __init__(self):
        ...
        self.can2=Canvas(self.fen,width=800,height=700,relief=GROOVE,cursor="pencil",bd=2)
        ...
        Button(self.fen,text='Valider',padx=60,bg='#5a5e6b',fg='orange',activebackground='brown',\
        ...
        self.liste_carrieres=Listbox(fen3a,width=25,height=20,bg="#cfc3ff")
...
class Accueil:
    def __init__(self):
        ...
        enter=Button(self.fen1,text='Enter',bg='#5a5e6b',font=text_font2,fg='#f5f5dc',cursor="hand1",\
                     padx=145,activebackground='#f5f5dc',highlightbackground='red',command=self.prem_lancement)
        enter.pack(side=LEFT)
        quitter=Button(self.fen1,text='Quit',bg='#5a5e6b',font=text_font2,fg='#f5f5dc',cursor="hand1",\
                       padx=145,activebackground='#f5f5dc',highlightbackground='red',command=self.fen1.destroy)
